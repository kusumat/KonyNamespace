...
voltmx.logger = {
    ...
    get currentLogLevel() {
        ...
        if (voltmx.logger.isNativeLoggerAvailable()) {
            var logLevelValue = VoltmxLogger.getLogLevel();
    ...
    set currentLogLevel(level) {
        currentLevel = level;
        if (voltmx.logger.isNativeLoggerAvailable())
            VoltmxLogger.setLogLevel(currentLevel.value);
    },

    isNativeLoggerAvailable: function() {
        if (typeof(VoltmxLogger) === 'undefined')
            return false;
        else
            return true;
    },
    ...
    activatePersistors: function(activatedList) {
        if (voltmx.logger.isNativeLoggerAvailable())
            VoltmxLogger.activatePersistors(activatedList);
    },
    deactivatePersistors: function(deactivatedList) {
        if (voltmx.logger.isNativeLoggerAvailable())
            VoltmxLogger.deactivatePersistors(deactivatedList);
    },
    subscribeLogListener: function(logListener) {
        if (voltmx.logger.isNativeLoggerAvailable()) {
            if(typeof(logListener) === 'function') {
                VoltmxLogger.subscribeLogListener(logListener);
            } else {
    ...
    setGlobalRequestParam: function(paramName, paramValue, paramType) {
        if (voltmx.logger.isNativeLoggerAvailable()) {
            VoltmxLogger.setGlobalRequestParam(paramName, paramValue, paramType);
        }
    },

    removeGlobalRequestParam: function(paramName, paramType) {
        if (voltmx.logger.isNativeLoggerAvailable()) {
            VoltmxLogger.removeGlobalRequestParam(paramName, paramType);
        }
    },

    resetGlobalRequestParams: function() {
        if (voltmx.logger.isNativeLoggerAvailable()) {
            VoltmxLogger.resetGlobalRequestParams();
        }
    },
...
voltmx.sdk.VMXObj = function(name, objectServiceName, namespace){
...
voltmx.sdk.VMXObjSvc = function (name) {
...
voltmx.logger.createNewLogger = function(loggerName, loggerConfig) {
    ...
    logMessage = function(loggerObj, logLevel, msg, params) {
        ...
        parseMessage = function(loggerObj, logLevel, msg, params) {
			var logLevelVal = (voltmx.logger.isNativeLoggerAvailable())? VoltmxLogger.getLogLevel() : voltmx.logger.currentLogLevel.value;
			...
			var metaData = {};
                ...
                if (voltmx.logger.isNativeLoggerAvailable()) {
                    if (!loggerObj.NativeLoggerObject) {
                        loggerObj.NativeLoggerObject = new VoltmxLogger.InitializeLogger(loggerObj.loggerName);
                        VoltmxLogger.setConfig(loggerObj.config);
                    }
                    if (loggerObj.NativeLoggerObject) {
    ...
    if (voltmx.logger.isNativeLoggerAvailable()) {
        loggerObj.NativeLoggerObject = new VoltmxLogger.InitializeLogger(loggerName);
        VoltmxLogger.setConfig(loggerObj.config);
    }
